...
def add_post(time, title, content):
    sql='''INSERT INTO data1(datetime, title, content) VALUES (%s, %s,%s)'''
    cur.execute(sql,(datetime, title, content))
    cnx.commit()
def edit_post1(b, datetime, title, content):
    sql='''UPDATE data1 SET datetime = %s, title = %s, content = %s WHERE id=%s'''
    cur.execute(sql,(datetime, title, content, b))
    cnx.commit()
...
def new_movie():
    ...
    add_post(datetime,title,content)
...
def edit_post(post_id):
    ...
    edit_post1(b,datetime, title, content)
    return redirect("/", code=302)
